...
abstract class ClientAbstract extends BoardAbstract
{
    ...
    public function getResourceUri($resource)
    {
        ...
        /* 
         * Modificação porca, nesse código lindo, mas não tem o que fazer no prazo que tenho. Diego - Braising - brg
         */
        
        switch ($endpoint) {
        	/* case (strpos($endpoint, 'api') != false):
        		$endpoint = str_replace('api.stelo.com.br', '200.142.203.218', $endpoint);
        	break; */
        	case (strpos($endpoint, 'carteira') != false):
        		$endpoint = str_replace('carteira.hml.stelo.com.br', '200.142.203.218', $endpoint);
        	break;
        	case (strpos($endpoint, 'sandbox') != false):
        		$endpoint = str_replace('sandbox.stelo.com.br', '200.142.203.223', $endpoint);
        	break;
        	case (strpos($endpoint, 'login') != false):
        		$endpoint = str_replace('login.hml.stelo.com.br', '200.142.203.223', $endpoint);
        	break;
        }
        
        /*Fim da modificação cretina*/
